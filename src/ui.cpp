...
void renderVisualization(ImDrawList *draw_list, GLuint tex, const std::vector<uint8_t> &block_buffer, float zoom,
    ...
    if (mouse_pos.x >= img_tl.x && mouse_pos.x < img_br.x && mouse_pos.y >= img_tl.y && mouse_pos.y < img_br.y) {
        ...
        if (idx < block_buffer.size()) {
            ...
            ImGui::SetTooltip("Sector %zu (%zu, %zu)\nEntropy: %.3f", current_block * block_size + idx, pixel_x,
                              pixel_y, entropy);
    ...
    if (uiState.highlighted_sector != SIZE_MAX) {
        ...
        if (sel_block == current_block) {
            ...
            if (local_idx < block_buffer.size()) {
                ...
                draw_list->AddLine(ImVec2(ruler_x, img_tl.y - 20), ImVec2(ruler_x, img_tl.y),
                                   IM_COL32(255, 0, 255, 255), 2.0f);
                float ruler_y = img_tl.y + sel_y * zoom;
                draw_list->AddLine(ImVec2(img_tl.x - 20, ruler_y), ImVec2(img_tl.x, ruler_y),
                                   IM_COL32(255, 0, 255, 255), 2.0f);
                // Lower right marking
                draw_list->AddLine(ImVec2(ruler_x + zoom, img_tl.y - 20), ImVec2(ruler_x + zoom, img_tl.y),
                                   IM_COL32(255, 0, 255, 255), 2.0f);
                draw_list->AddLine(ImVec2(img_tl.x, ruler_y + zoom), ImVec2(img_tl.x - 20, ruler_y + zoom),
                                   IM_COL32(255, 0, 255, 255), 2.0f);
            }
        }
    }
...
void handleFileDialogs(UiState &uiState, AppState &appState, IGFD::FileDialogConfig &config) {
    ...
    if (ImGuiFileDialog::Instance()->Display("GenerateFileDlg", ImGuiWindowFlags_NoCollapse, minSize, maxSize)) {
        if (ImGuiFileDialog::Instance()->IsOk()) {
            ...
            appState.cacheThread =
                std::thread([fileToCache, cacheFile]() { generateCacheThreaded(fileToCache, cacheFile); });
