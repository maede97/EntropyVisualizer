...
void upload_block(GLuint tex, const std::vector<uint8_t> &raw,
...
void renderHexViewWindow(UiState &uiState, const AppState &appState) {
    if (uiState.showHexView) {
        ...
        if (ImGui::Begin("Hex View", &uiState.showHexView)) {
            ImGui::Text("Sector %zu (0x%zX)", uiState.currentSectorIndex,
                        uiState.currentSectorIndex * 512);
            ImGui::Separator();
            ...
            if (uiState.currentSectorData.empty()) {
                ...
                std::map<size_t, std::pair<uint32_t, int>>
                    highlightMap; // color, priority
                if (appState.hexDisplayFeatureManager) {
                    for (const auto *feature :
                         appState.hexDisplayFeatureManager->getFeatures()) {
                        if (appState.featureEnabled.count(feature->getName()) &&
                            appState.featureEnabled.at(feature->getName())) {
                            auto highlights = feature->getHighlights(
                                uiState.currentSectorData,
                                uiState.currentSectorIndex);
                            int priority = feature->getPriority();
                            for (const auto &h : highlights) {
                                auto it = highlightMap.find(h.offset);
                                if (it == highlightMap.end() ||
                                    it->second.second < priority) {
                                    highlightMap[h.offset] = {h.color,
                                                              priority};
                                }
                            }
                        }
                ...
                for (size_t i = 0; i < uiState.currentSectorData.size();
                     i += 16) {
                    ImGui::Text("%08zX: ", i);
                    ImGui::SameLine();
                    for (size_t j = 0;
                        ...
                        if (highlightMap.count(offset)) {
                            ImGui::TextColored(
                                ImColor(highlightMap[offset].first), "%02X ",
                                uiState.currentSectorData[offset]);
                        } else {
                            ImGui::Text("%02X ",
                                        uiState.currentSectorData[offset]);
                        }
...
void renderSearchWindow(UiState &uiState, AppState &appState,
    ...
    if (uiState.showSearchWindow) {
        ...
        if (ImGui::Begin("Search Sectors", &uiState.showSearchWindow)) {
            const char *items[] = {"Max Entropy (at most)",
                                   "Min Entropy (at least)", "Entropy Range"};
            ImGui::Combo("Search Type", &uiState.search_type, items,
                         IM_ARRAYSIZE(items));
            ...
            if (ImGui::Button("Find")) {
                ...
                size_t start = (uiState.highlighted_sector == SIZE_MAX
                                    ? 0
                                    : uiState.highlighted_sector + 1);
                if (uiState.search_type == 0) {
                    // max entropy (at most)
                    for (size_t i = start; i < appState.all_cache_data.size();
                         ++i) {
                ...
                if (found_index != SIZE_MAX) {
                    ...
                    size_t block_size =
                        appState.block_width * appState.block_height;
...
void renderVisualization(ImDrawList *draw_list, GLuint tex,
                         const std::vector<uint8_t> &block_buffer, float zoom,
                         ImVec2 pan_offset, size_t current_block,
                         size_t block_size, size_t block_width,
                         size_t block_height, UiState &uiState,
                         std::function<void(size_t)> loadHexData) {
    ...
    for (size_t i = 0; i <= block_width; i += 32) {
        ...
        if (x >= img_tl.x && x <= img_br.x) {
            draw_list->AddLine(ImVec2(x, img_tl.y - 10), ImVec2(x, img_tl.y),
                               IM_COL32(255, 255, 255, 128), 1.0f);
            draw_list->AddText(ImVec2(x - 10, img_tl.y - 25),
    ...
    for (size_t j = 0; j <= block_height; j += 32) {
        ...
        if (y >= img_tl.y && y <= img_br.y) {
            draw_list->AddLine(ImVec2(img_tl.x - 10, y), ImVec2(img_tl.x, y),
                               IM_COL32(255, 255, 255, 128), 1.0f);
            draw_list->AddText(ImVec2(img_tl.x - 50, y - 7),
                               IM_COL32(255, 255, 255, 255),
    ...
    for (size_t i = 0; i <= block_width; i += 32) {
        float x = img_tl.x + i * zoom;
        if (x >= img_tl.x && x <= img_br.x) {
            draw_list->AddLine(ImVec2(x, img_br.y - 10), ImVec2(x, img_br.y),
                               IM_COL32(255, 255, 255, 128), 1.0f);
            draw_list->AddText(ImVec2(x - 10, img_br.y + 5),
                               IM_COL32(255, 255, 255, 255),
                               std::to_string(i).c_str());
        }
    }
    ...
    for (size_t j = 0; j <= block_height; j += 32) {
        float y = img_tl.y + j * zoom;
        if (y >= img_tl.y && y <= img_br.y) {
            draw_list->AddLine(ImVec2(img_br.x - 10, y), ImVec2(img_br.x, y),
                               IM_COL32(255, 255, 255, 128), 1.0f);
            draw_list->AddText(ImVec2(img_br.x + 5, y - 7),
                               IM_COL32(255, 255, 255, 255),
                               std::to_string(j).c_str());
        }
    }
    ...
    if (mouse_pos.x >= img_tl.x && mouse_pos.x < img_br.x &&
        mouse_pos.y >= img_tl.y && mouse_pos.y < img_br.y) {
        float local_x = (mouse_pos.x + pan_offset.x) / zoom;
        float local_y = (mouse_pos.y + pan_offset.y) / zoom;

        size_t pixel_x =
            (size_t)std::max(0.0f, std::min(local_x, (float)block_width - 1));
        size_t pixel_y =
            (size_t)std::max(0.0f, std::min(local_y, (float)block_height - 1));
        ...
        if (idx < block_buffer.size()) {
            ...
            ImGui::SetTooltip("Sector %zu (%zu, %zu)\nEntropy: %.3f",
                              current_block * block_size + idx, pixel_x,
                              pixel_y, entropy);

            // Highlight hovered pixel
            ImVec2 highlight_tl =
                ImVec2(img_tl.x + pixel_x * zoom, img_tl.y + pixel_y * zoom);
            ImVec2 highlight_br =
                ImVec2(highlight_tl.x + zoom, highlight_tl.y + zoom);
            draw_list->AddRect(highlight_tl, highlight_br,
    ...
    if (uiState.highlighted_sector != SIZE_MAX) {
        ...
        if (sel_block == current_block) {
            ...
            if (local_idx < block_buffer.size()) {
                ImVec2 sel_tl =
                    ImVec2(img_tl.x + sel_x * zoom, img_tl.y + sel_y * zoom);
                ImVec2 sel_br = ImVec2(sel_tl.x + zoom, sel_tl.y + zoom);
                draw_list->AddRect(sel_tl, sel_br, IM_COL32(255, 0, 255, 255),
                                   0.0f, 0, 3.0f);

                // Highlight in rulers
                float ruler_x = img_tl.x + sel_x * zoom;
                draw_list->AddLine(ImVec2(ruler_x, img_tl.y - 20),
                                   ImVec2(ruler_x, img_tl.y),
                                   IM_COL32(255, 0, 255, 255), 2.0f);
                float ruler_y = img_tl.y + sel_y * zoom;
                draw_list->AddLine(ImVec2(img_tl.x - 20, ruler_y),
                                   ImVec2(img_tl.x, ruler_y),
                                   IM_COL32(255, 0, 255, 255), 2.0f);
                // Lower right marking
                draw_list->AddLine(ImVec2(ruler_x + zoom, img_tl.y - 20),
                                   ImVec2(ruler_x + zoom, img_tl.y),
                                   IM_COL32(255, 0, 255, 255), 2.0f);
                draw_list->AddLine(ImVec2(img_tl.x, ruler_y + zoom),
                                   ImVec2(img_tl.x - 20, ruler_y + zoom),
                                   IM_COL32(255, 0, 255, 255), 2.0f);
            }
        }
    }
...
void handleFileDialogs(UiState &uiState, AppState &appState,
    ...
    if (ImGuiFileDialog::Instance()->Display(
            "OpenCacheDlg", ImGuiWindowFlags_NoCollapse, minSize, maxSize)) {
        if (ImGuiFileDialog::Instance()->IsOk()) {
            ...
            if (appState.file) {
                ...
                appState.file.read(
                    reinterpret_cast<char *>(appState.all_cache_data.data()),
                    appState.file_size);
                appState.current_block = 0;
                appState.block_slider = 0;
                appState.redrawBlock = true;
                appState.promptForSource =
                    true; // Always prompt for source when opening cache
    ...
    if (ImGuiFileDialog::Instance()->Display(
            "GenerateFileDlg", ImGuiWindowFlags_NoCollapse, minSize, maxSize)) {
        if (ImGuiFileDialog::Instance()->IsOk()) {
            std::string fileToCache =
                ImGuiFileDialog::Instance()->GetFilePathName();
            ...
            std::string filename_only =
                fs::path(fileToCache).filename().string();
            std::string cacheFile =
                (fs::temp_directory_path() / (filename_only + ".cache.bin"))
                    .string();
            appState.lastCacheFile = cacheFile;
            appState.originalFile = fileToCache;
            appState.cacheThread = std::thread([fileToCache, cacheFile]() {
                generateCacheThreaded(fileToCache, cacheFile);
            });
    ...
    if (ImGuiFileDialog::Instance()->Display(
            "SelectSourceDlg", ImGuiWindowFlags_NoCollapse, minSize, maxSize)) {
        if (ImGuiFileDialog::Instance()->IsOk()) {
            ...
            if (source) {
                ...
                size_t expected_cache_size =
                    (source_size + 511) / 512; // SECTOR_SIZE = 512
